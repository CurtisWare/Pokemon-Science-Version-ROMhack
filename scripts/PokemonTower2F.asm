...
IF DEF(_DEBUG)
	...
	ret nz
ENDC
	CheckEvent EVENT_BEAT_POKEMON_TOWER_COQUI
	ret nz
	...
	call PlayMusic
	ResetEvent EVENT_POKEMON_TOWER_COQUI_ON_LEFT
	ld a, [wCoordIndex]
	cp $1
	ld a, PLAYER_DIR_UP
	ld b, SPRITE_FACING_DOWN
	jr nz, .asm_60544
; the coqui is on the left side and the player is on the right side
	SetEvent EVENT_POKEMON_TOWER_COQUI_ON_LEFT
...
PokemonTower2Script1:
	...
	ld a, $f0
	ld [wJoyIgnore], a
	SetEvent EVENT_BEAT_POKEMON_TOWER_COQUI
	ld a, $1
	ldh [hSpriteIndexOrTextID], a
	call DisplayTextID
	ld de, MovementData_605b2
	CheckEvent EVENT_POKEMON_TOWER_COQUI_ON_LEFT
	jr nz, .asm_60589
	callfar Func_f1e22
...
PokemonTower2Script2:
	...
	bit 0, a
	ret nz
	ld a, HS_POKEMON_TOWER_2F_COQUI
