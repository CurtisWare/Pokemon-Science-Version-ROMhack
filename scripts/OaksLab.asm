...
OaksLabScript8:
	ld a, $1
	ldh [hSpriteIndexOrTextID], a
	ld de, .CoquiPushesPlayerAwayFromEeveeBall
	call MoveSprite
	ld a, $9
	ld [wOaksLabCurScript], a
	ret

.CoquiPushesPlayerAwayFromEeveeBall
	db $00
	db $07
	db $07
	db $07
...
OaksLabScript13:
	...
	ld a, $1
	ld [wTrainerNo], a
	ld hl, OaksLabCoquiDefeatedText
	ld de, OaksLabCoquiBeatYouText
...
OaksLabScript14:
	...
	; If you beat your coqui here, his Eevee will evolve into
	; Jolteon if you beat him on Route 22, or Flareon if you
	; skip or lose that battle.
	; Otherwise, it will evolve into Vaporeon.
	ld a, [wBattleResult]
	and a
	ld b, COQUI_STARTER_VAPOREON
	jr nz, .got_coqui_starter
	ld b, COQUI_STARTER_FLAREON
.got_coqui_starter
	ld a, b
	ld [wCoquiStarter], a
	...
	ld [wSprite01StateData1FacingDirection], a
	predef HealParty
	SetEvent EVENT_BATTLED_COQUI_IN_OAKS_LAB
	ld a, $f
	ld [wOaksLabCurScript], a
	ret
...
OaksLabScript15:
	...
	ldh [hSpriteIndexOrTextID], a
	call DisplayTextID
	farcall Music_CoquiAlternateStart
	ld a, $1
	ldh [hSpriteIndex], a
	ld de, .OaksLabMovement_CoquiWalksOut1
	call MoveSprite
	ld a, [wXCoord]
	cp 4
	jr nz, .asm_1c6bb
...
.OaksLabMovement_CoquiWalksOut1
...
OaksLabScript16:
	ld a, [wd730]
	bit 0, a
	jr nz, .checkCoquiPosition
	ld a, ~(A_BUTTON | B_BUTTON)
	ld [wJoyIgnore], a
	ld a, HS_OAKS_LAB_COQUI
	ld [wMissableObjectIndex], a
	predef HideObject
	call PlayDefaultMusic
	ld a, $11
	ld [wOaksLabCurScript], a
	ret
; make the player keep facing the coqui as he walks away
.checkCoquiPosition
	ld a, [wNPCNumScriptedSteps]
	cp $5
...
OaksLabScript21:
	...
	ld [wMissableObjectIndex], a
	predef HideObject
	SetEvent EVENT_1ST_ROUTE22_COQUI_BATTLE
	ResetEventReuseHL EVENT_2ND_ROUTE22_COQUI_BATTLE
	SetEventReuseHL EVENT_ROUTE22_COQUI_WANTS_BATTLE
	ld a, HS_ROUTE_22_COQUI_1
	ld [wMissableObjectIndex], a
	predef ShowObject
	xor a
	ld [wJoyIgnore], a
...
.asm_1c9ec
	...
	jr nz, .asm_1ca4a
	CheckEventReuseA EVENT_BATTLED_COQUI_IN_OAKS_LAB
	jr nz, .asm_1ca2b
	ld a, [wd72e]
	bit 3, a
	jr nz, .asm_1ca23
...
OaksLabText13:
	text_asm
	ld hl, OaksLabCoquiWaitingText
	call PrintText
	jp TextScriptEnd

OaksLabCoquiWaitingText:
	text_far _OaksLabCoquiWaitingText
	text_end
...
OaksLabText15:
	text_asm
	ld hl, OaksLabCoquiInterjectionText
	call PrintText
	jp TextScriptEnd

OaksLabCoquiInterjectionText:
	text_far _OaksLabCoquiInterjectionText
	text_end
...
OaksLabText17:
	text_asm
	ld hl, OaksLabCoquiTakesText1
	call PrintText
	ld hl, OaksLabCoquiTakesText2
	call PrintText
	ld hl, OaksLabCoquiTakesText3
	call PrintText
	ld hl, OaksLabCoquiTakesText4
	call PrintText
	ld hl, OaksLabCoquiTakesText5
	call PrintText
	jp TextScriptEnd

OaksLabCoquiTakesText1:
	text_far _OaksLabCoquiTakesText1
	text_end

OaksLabCoquiTakesText2:
	text_far _OaksLabCoquiTakesText2
	sound_get_key_item
	text_end

OaksLabCoquiTakesText3:
	text_far _OaksLabCoquiTakesText3
	text_end

OaksLabCoquiTakesText4:
	text_far _OaksLabCoquiTakesText4
	text_end

OaksLabCoquiTakesText5:
	text_far _OaksLabCoquiTakesText5
	text_end
...
OaksLabText11:
	text_asm
	ld hl, OaksLabCoquiChallengeText
	call PrintText
	jp TextScriptEnd

OaksLabCoquiChallengeText:
	text_far _OaksLabCoquiChallengeText
	text_end

OaksLabCoquiDefeatedText:
	text_far _OaksLabText_1d3be
	text_end

OaksLabCoquiBeatYouText:
	text_far _OaksLabText_1d3c3
	text_end

OaksLabText12:
	text_asm
	ld hl, OaksLabCoquiToughenUpText
	call PrintText
	jp TextScriptEnd

OaksLabCoquiToughenUpText:
	text_far _OaksLabCoquiToughenUpText
	text_end
