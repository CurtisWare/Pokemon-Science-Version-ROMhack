...
VermilionGym_ScriptPointers:
	...
	dw EndTrainerBattle
	dw VermilionGymLTPeonyPostBattle

VermilionGymLTPeonyPostBattle:
	ld a, [wIsInBattle]
	cp $ff ; did we lose?
...
VermilionGym_TextPointers:
	...
	dw LTPeonyThunderBadgeInfoText
	dw ReceivedTM24Text
	dw TM24NoRoomText
...
LTPeonyText:
	text_asm
	CheckEvent EVENT_BEAT_LT_PEONY
	jr z, .beforeBeat
	...
	jr .done
.afterBeat
	ld hl, LTPeonyPostBattleAdviceText
	call PrintText
	jr .done
.beforeBeat
	ld hl, LTPeonyPreBattleText
	call PrintText
	ld hl, wd72d
	set 6, [hl]
	set 7, [hl]
	...
	xor a
	ldh [hJoyHeld], a
	ld a, $3 ; set script index to LT Peony post-battle script
	ld [wVermilionGymCurScript], a
	ld [wCurMapScript], a
.done
	jp TextScriptEnd

LTPeonyPreBattleText:
	text_far _LTPeonyPreBattleText
	text_end

LTPeonyPostBattleAdviceText:
	text_far _LTPeonyPostBattleAdviceText
	text_end

LTPeonyThunderBadgeInfoText:
	text_far _LTPeonyThunderBadgeInfoText
	text_end
