...
.gymleader
	ld a, [wTrainerClass]
	cp COQUI3 ; final battle against coqui
	jr nz, .notcoqui
	ld b, MUSIC_DEFEATED_GYM_LEADER
	ld hl, wFlags_D733
	set 1, [hl]
...
; prints appropriate lose message, sets carry flag if player blacked out (special case for initial coqui fight)
HandlePlayerBlackOut:
	ld a, [wLinkState]
	cp LINK_STATE_BATTLING
	jr z, .notCoqui1Battle
	ld a, [wCurOpponent]
	cp OPP_COQUI1
	jr nz, .notCoqui1Battle
	hlcoord 0, 0  ; coqui 1 battle
	lb bc, 8, 21
	call ClearScreenArea
	call ScrollTrainerPicAfterBattle
	ld c, 40
...
.nextTypePair2
	...
	jr .loop
.done
	; 40% chance for Colt's Dewgong to ignore type effectiveness?
	ld a, [wTrainerClass]
	cp COLT
	jr nz, .ok
	ld a, [wEnemyMonSpecies]
